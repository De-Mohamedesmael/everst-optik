...
<tr id="row_batch_details_{{ $row_count }}" class="row_batch_details row_batch_details_{{ $row_count }}"
    ...
    {{-- <td colspan="1"><img
            src="@if (!empty($products->getFirstMediaUrl('products'))){{$products->getFirstMediaUrl('products')}}@else{{asset('/uploads/'.session('logo'))}}@endif"
    ...
    <td colspan="1">
        ...
        <input type="text" data-val="0"
            ...
            data-id="{{ $product->product_id }}" min=1 name="batch_row[{{ $index }}][batch_quantity]" required value="0"
            index_id="">
        {{-- {!! Form::label('', __('lang.days_before_the_expiry_date'), []) !!}
        {!! Form::text('batch_row['.$i.'][expiry_warning]', null, ['class' => 'form-control
        days_before_the_expiry_date']) !!} --}}
    </td>
    <td colspan="1">
        <div style="width: 100%;height: 100%;" class="d-flex justify-content-center align-items-start">
            ...
            <input type="text"
                ...
                data-id="{{ $product->product_id }}" name="batch_row[{{ $index }}][batch_purchase_price]" required
